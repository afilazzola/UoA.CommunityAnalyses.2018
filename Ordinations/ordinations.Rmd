...
We can plot the dendrogram to be more aesthetically pleasing using the `cluster` and `factoextra` packages. Although this is not necessary. Here we can clearly highlight the clustered groups that we generated. We can also use the function cophenetic to estimate the dissimilarity from the tree based on all the points. 
```{r warning=FALSE, message=FALSE}
library(cluster)
library(factoextra)
...
## Function cophenetic finds this estimated dissimilarity from a tree for evey pair of points. 
