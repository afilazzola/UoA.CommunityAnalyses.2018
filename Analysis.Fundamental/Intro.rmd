...
kable(data[1:20,1:8]) %>% ## select only the first 20 rows and 8 columns
...
data <- read.csv("PracticeData2.csv")

kable(data[1:20,]) %>% ## select only the first 20 rows 
  kable_styling(bootstrap_options = c("striped", "hover"))
```
...
data.wide <- data %>% spread(Species, Abundance)
...
Using the same dataset as before, let's calculate the mean, standard deviation, and n for my exclosure treatments. 

```{r}

data.summary <- data.long %>% group_by(Exclosure) %>% summarize(avg=mean(Abundance), stdev=sd(Abundance), n=length(Abundance))
...
data.summary <- data.long %>% group_by(Microsite, Exclosure) %>% summarize(avg=mean(Abundance), stdev=sd(Abundance), n=length(Abundance))
